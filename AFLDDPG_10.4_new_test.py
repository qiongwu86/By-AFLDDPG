...
for ep in tqdm(range(MAX_EPISODE)):  # (多少个episode)
    ...
    for i in range(args.num_users):
        ...
        #localtime[2] = 10000 * CPUcycles / delta[i] - 0.5
    ...
    for j in range(MAX_EPISODE_LEN):
        ...
        # server_model = LocalUpdate(args=args, dataset=trdata, idxs=usgrp[4], logger=logger)  # 先暂时将测试样本数定为用第五个用户的数据量大小
        ...
        for aa in user_id:
            if P_lamda11xx[aa] >= 0.5:
                ...
                # 无bzt攻击
                w, loss, localmodel = locmdl.update_weights(model=copy.deepcopy(glmodel), global_round=ep, index=aa,lt= localtime[aa],blt=beta_lt[aa],ct=c_c[aa],bct=beta_ct[aa])


                # w, loss, localmodel = locmdl.update_weights(model=copy.deepcopy(glmodel), global_round=ep, index=aa)
                locloss.append(loss)
